...
public class ContentsExtractor {
     ...
     *
     * <p>It ignores all the spaces between hash signs and a title, trailing spaces are also ignored.
     */
    private static final Pattern pattern = Pattern.compile(" *(?<level>#+) *(?<title>[^#].*?) *");

    /**
     * Takes all the {@link Header headers} from the {@code lines}
     * and returns them as {@link Contents} instance.
     *
